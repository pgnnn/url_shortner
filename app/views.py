...
def createShortURL(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #         random_chars += random.choice(random_chars_list)
            d = datetime.now()
            hash = hashlib.md5(original_website.encode()).hexdigest()[:8]
            s = ShortURL(original_url=original_website, short_url=hash, time_date_created=d)
            s.save()
            return render(request, 'urlcreated.html', {'chars':hash})
